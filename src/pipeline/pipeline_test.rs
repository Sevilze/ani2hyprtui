...
mod tests {
    ...
    fn test_full_pipeline_win2xcur_to_xcur2png() {
        ...
        if win_cursor_path.exists() {
            ...
            let options = win2xcur::converter::ConversionOptions::new()
                .with_scale(1.0);
            ...
            assert!(result.is_ok(), "Failed to convert .cur to Xcursor: {:?}", result.err());
            ...
            let extracted_files = xcur2png::extract_to_pngs(
                &xcur_output,
                &png_dir,
                &extract_options
            );
            
            assert!(extracted_files.is_ok(), "Failed to extract PNGs: {:?}", extracted_files.err());
            ...
            assert!(config_content.contains("crosshair_000.png"), "Config missing PNG reference");
            ...
            eprintln!("Warning: Sample cursor file not found at {}", win_cursor_path.display());
